...
# Specify the program language. It can be "fr_FR", "zh_CN", or any folder in Fbleee-locales.
# Defaults to English.
language = "zh_CN"
...
class FbleeeSrcPlugin(idaapi.plugin_t):
    flags = 0
    explain_action_name = "Fbleee:explain_function"
    explain_menu_path = "Edit/Fbleee/" + _("Explain function")
    rename_action_name = "Fbleee:rename_function"
    rename_menu_path = "Edit/Fbleee/" + _("Rename variables")
    wanted_name = 'Fbleee'
    wanted_hotkey = ''
    ...
    def finish_populating_widget_popup(self, form, popup):
        ...
        if idaapi.get_widget_type(form) == idaapi.BWN_PSEUDOCODE:
            idaapi.attach_action_to_popup(
                form, popup, FbleeeSrcPlugin.explain_action_name, "Fbleee/")
            idaapi.attach_action_to_popup(
                form, popup, FbleeeSrcPlugin.rename_action_name, "Fbleee/")
...
def comment_callback(address, view, response):
    ...
    # Add the response as a comment in IDA, but preserve any existing non-Fbleee comment
    comment = idc.get_func_cmt(address, 0)
    comment = re.sub(r'----- ' + _("Comment generated by Fbleee") + ' -----.*?----------------------------------------',
                     r"",
                     comment,
                     flags=re.DOTALL)

    idc.set_func_cmt(address, '----- ' + _("Comment generated by Fbleee") +
                     f" -----\n\n"
                     f"{response.strip()}\n\n"
                     f"----------------------------------------\n\n"
                     f"{comment.strip()}", 0)
...
def updateImage(query,API_Key):
    ...
    if (input_str is None or input_str == "exit"):
        sys.exit()
    m = 'https://api.chatanywhere.com.cn/v1/chat/completions'
    tinydict2 = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "Hello!"}]
    }
    tinydict2["messages"][0]["content"] = input_str
    tinydict.update(tinydict2)
    # todo python json 转bytes
    json_data = json.dumps(tinydict)
    bytes_data = bytes(json_data, 'utf-8')
    x = requests.post(m, data=bytes_data, proxies=proxies, headers=headers)
    return_str = x.text
...
def query_model(query, cb, max_tokens=2500):
    ...
    """
    try:
        response = updateImage(query,API_Key)
